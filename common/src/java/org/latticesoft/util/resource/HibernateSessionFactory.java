 ...
 * <tt>Session</tt> and <tt>Transaction</tt> should be used, for resource
 * local transactions or BMT, or if CMT with automatic <tt>Session</tt>
 * handling is enabled.
 * <p>
 * To keep your DAOs free from any of this, just call
 * <tt>HibernateUtil.getCurrentSession()</tt> in the constructor of each DAO.,
 * The recommended way to set resource local or BMT transaction boundaries is an
 * interceptor, or a request filter.
 * <p>
 ...
 * no-argument constructor. You can call <tt>getInterceptor()</tt> if you need
...
public class HibernateSessionFactory {
	...
	static {
		...
		try {
			...
			if (interceptorName != null) {
				Class interceptorClass = HibernateSessionFactory.class.getClassLoader().loadClass(interceptorName);
	...
	public static SessionFactory getSessionFactory() {
		...
		if (sfName != null) {
if (log.isDebugEnabled()) { log.debug("Looking up SessionFactory in JNDI."); }
			try {
...
if (log.isDebugEnabled()) { log.debug("Shutting down Hibernate."); }
...
if (log.isDebugEnabled()) { log.debug("Using current Configuration for rebuild."); }
...
if (log.isDebugEnabled()) { log.debug("Rebuilding the SessionFactory from given Configuration."); }
	...
	public static void closeSession() {
		if (useThreadLocal) {
			...
			if (s != null && s.isOpen()) {
if (log.isDebugEnabled()) { log.debug("Closing Session of this thread."); }
				s.close();
			}
		} else {
			if (log.isWarnEnabled()) { log.warn("Using CMT/JTA, intercepted superfluous close call."); }
		}
	}
	...
	public static void beginTransaction() {
		if (useThreadLocal) {
			Transaction tx = (Transaction) threadTransaction.get();
			if (tx == null) {
if (log.isDebugEnabled()) { log.debug("Starting new database transaction in this thread."); }
				tx = getCurrentSession().beginTransaction();
				threadTransaction.set(tx);
			}
		} else {
if (log.isWarnEnabled()) { log.warn("Using CMT/JTA, intercepted superfluous tx begin call."); }
		}
	}
	...
	public static void commitTransaction() {
		if (useThreadLocal) {
			...
			try {
				if (tx != null && !tx.wasCommitted() && !tx.wasRolledBack()) {
if (log.isDebugEnabled()) { log.debug("Committing database transaction of this thread."); }
					tx.commit();
				}
				threadTransaction.set(null);
			...
			if (log.isWarnEnabled()) { log.warn("Using CMT/JTA, intercepted superfluous tx commit call."); }
		}
	}
	...
	public static void rollbackTransaction() {
		if (useThreadLocal) {
			...
			try {
				threadTransaction.set(null);
				if (tx != null && !tx.wasCommitted() && !tx.wasRolledBack()) {
if (log.isDebugEnabled()) { log.debug("Tyring to rollback database transaction of this thread."); }
					tx.rollback();
if (log.isDebugEnabled()) { log.debug("Database transaction rolled back."); }
				}
			} catch (RuntimeException ex) {
			...
			if (log.isWarnEnabled()) { log.warn("Using CMT/JTA, intercepted superfluous tx rollback call."); }
		}
	}
	...
	public static void reconnect(Session session) {
		if (useThreadLocal) {
if (log.isDebugEnabled()) { log.debug("Reconnecting Session to this thread."); }
			session.reconnect();
			threadSession.set(session);
		} else {
if (log.isErrorEnabled()) { log.error("Using CMT/JTA, intercepted not supported reconnect call."); }
		}
	}
...
if (log.isDebugEnabled()) { log.debug("Disconnecting Session from this thread."); }
				session.disconnect();
			}
			return session;
		} else {
if (log.isErrorEnabled()) { log.error("Using CMT/JTA, intercepted not supported disconnect call."); }
			return null;
		}
	}
...
if (log.isDebugEnabled()) { log.debug("Setting new global Hibernate interceptor and restarting."); }
		configuration.setInterceptor(interceptor);
